...
public class adapPersonRole extends RecyclerView.Adapter<adapPersonRole.AdapterMemberGtoup> {
    ...
    public List<Basic_PersonRoles> lList;
    public adapPersonRole(List<Basic_PersonRoles> lList, Context context){
        this.lList = lList;
    ...
    public void onBindViewHolder(final AdapterMemberGtoup holder, final int position) {
        ...
        lblTitle.setText(lList.get(position).PersonRoleTitle);
        try {
            ByteArrayOutputStream stream = null;
            try {
                Bitmap bmp=Bitmap.createBitmap(100,100, Bitmap.Config.ARGB_8888);
                Canvas canvas=new Canvas(bmp);
                canvas.drawColor(Color.parseColor("#" + lList.get(position).PersonRoleColor));
        ...
        cardViewMain.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                fragPersonRole.onEditor(lList.get(position));
                return false;
            }
        });
    }
    public void MoveToDown(int position) {
        ...
        if(position != lList.size() - 1){
            Basic_PersonRoles itemA = lList.get(position);
            Basic_PersonRoles itemB = lList.get(NextPosition);
    ...
    public void MoveToUp(int position) {
        ...
        if(position != 0){
            Basic_PersonRoles itemA = lList.get(position);
            Basic_PersonRoles itemB = lList.get(PreviusePosition);
            lList.set(position, itemB);
