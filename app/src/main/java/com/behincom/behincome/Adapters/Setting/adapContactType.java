...
public class adapContactType extends RecyclerView.Adapter<adapContactType.AdapterMemberGtoup> {
    ...
    public List<Basic_ContactTypes> lList;
    public adapContactType(List<Basic_ContactTypes> lList, Context context){
        this.lList = lList;
    ...
    public void onBindViewHolder(final AdapterMemberGtoup holder, final int position) {
        ...
        lblTitle.setText(lList.get(position).ContactTypeTitle);
        try {
            ByteArrayOutputStream stream = null;
            try {
                Bitmap bmp=Bitmap.createBitmap(100,100, Bitmap.Config.ARGB_8888);
                Canvas canvas=new Canvas(bmp);
                canvas.drawColor(Color.parseColor("#" + lList.get(position).ContactTypeColor));
        ...
        cardViewMain.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                fragContactType.onEditor(lList.get(position));
                return false;
            }
        });
    }
    public void MoveToDown(int position) {
        ...
        if(position != lList.size() - 1){
            Basic_ContactTypes itemA = lList.get(position);
            Basic_ContactTypes itemB = lList.get(NextPosition);
    ...
    public void MoveToUp(int position) {
        ...
        if(position != 0){
            Basic_ContactTypes itemA = lList.get(position);
            Basic_ContactTypes itemB = lList.get(PreviusePosition);
            lList.set(position, itemB);
